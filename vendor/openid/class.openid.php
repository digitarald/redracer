<?
/**
 * HACKED by Harald Kirschner <mail@digitarald.de>
 * Removed $_GET usage from ValidateWithServer
 */
...
//*/
	...
	function ValidateWithServer($get){
		$params = array(
		...
		$arr_signed = explode(",",str_replace('sreg.','sreg_', $get['openid_signed']));
		for ($i=0; $i<count($arr_signed); $i++){
			$s = str_replace('sreg_','sreg.', $arr_signed[$i]);
			$c = $get['openid_' . $arr_signed[$i]];
			// if ($c != ""){
