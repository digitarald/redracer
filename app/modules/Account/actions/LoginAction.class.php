...
class Account_LoginAction extends RedBaseAction
{
	...
	public function executeWrite(AgaviRequestDataHolder $rd)
	{
		...
		$url = $rd->getParameter('openid_identifier');
		...
		if (!$auth_request) {
			 ...
			 */
			$this->vm->setError('openid_identifier',  'Authentication error, not a valid OpenID identifier!');
		...
		if ($auth_request->shouldSendRedirect() ) {
			...
			if (Auth_OpenID::isFailure($redirect_url) ) {
				$this->vm->setError('openid_identifier',  'Could not redirect to server: ' . $redirect_url->message);
				return $this->handleError($rd);
			} else {
				// Send redirect.
			...
			if (Auth_OpenID::isFailure($form_html) ) {
				$this->vm->setError('openid_identifier',  'Could not redirect to server: ' . $form_html->message);
				return $this->handleError($rd);
			} else {
